const forLoopsModule = require('./loop.for.module');
const forofLoopsModule = require('./loop.forof.module');
const arrModule = require('./loop.array.module');
...
describe('Array tests', function () {
    it('array foreach should return the right array', function () {
        let val = arrModule.arrForEeach(['ajhfb', 23, function () {}, () => {}], addCharacter);
    ...
    it('array every should return the right array', function () {
        let val = arrModule.arrEveryReturnNewValue(['ajhfb', 23, function () {}, () => {}], addCharacter);
    ...
    it('array every should verify the data', function () {
        assert.strictEqual(arrModule.arrEveryVerify([1, 5, 9, 4, 3], isNumber), true);
        assert.strictEqual(arrModule.arrEveryVerify([1, 5, 9, () => {}, 3], isNumber), false);
        assert.strictEqual(arrModule.arrEveryVerify([1, 5, '9', 4, 3], isNumber), false);
        assert.strictEqual(arrModule.arrEveryVerify([1, 5, 9, 6, 3], isNumber), true);
    });

    it('array some should verify the data', function () {
        assert.strictEqual(arrModule.arrSomeVerify([1, 5, 9, 4, 3], isString), false);
        assert.strictEqual(arrModule.arrSomeVerify([1, 5, 9, () => {}, 3], isString), false);
        assert.strictEqual(arrModule.arrSomeVerify([1, 5, 'thisisaTest', 4, 3], isString), true);
        assert.strictEqual(arrModule.arrSomeVerify([1, 5, 9, 6, 3], isString), false);
    });
